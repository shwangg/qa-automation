class AuthorityPage
  ...
  def verify_display_name(name, index)
    wait_until(Config.short_wait, "Expected display name '#{name}', but got '#{element_value(term_display_name_input index)}'") do
      text_values_match?(name, element_value(term_display_name_input index))
    end
  end
