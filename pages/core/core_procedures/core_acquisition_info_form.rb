...
module CoreAcquisitionInfoForm
  ...
  def acquis_date_input_locator(index)
    structured_date_input_locator([fieldset(AcquisitionData::ACQUIS_DATE_GRP.name, index)])
  end
  ...
  def enter_acquisition_dates(data_set)
    ...
    acquis_dates && acquis_dates.each do |date|
      ...
      add_button_locator = add_button_locator([fieldset(AcquisitionData::ACQUIS_DATE_GRP.name)])
      hit_escape
      wait_for_element_and_click add_button_locator unless index.zero?
      wait_for_element_and_type(acquis_date_input_locator(index), date[AcquisitionData::ACQUIS_DATE.name])
      hit_enter
    end
  end
  ...
  def acquis_method_input_locator
    input_locator([], AcquisitionData::ACQUIS_METHOD.name)
  end
  ...
  def select_acquisition_method(data_set)
    ...
    if acquis_method
      ...
      method_options_locator = input_options_locator([], AcquisitionData::ACQUIS_METHOD.name)
      hit_escape
  ...
  def acquis_note_input_locator
    text_area_locator([], AcquisitionData::ACQUIS_NOTE.name)
  end

  # Enters an acquisition note
  # @param [Hash] data_set
  def enter_acquisition_note(data_set)
    acquis_note = data_set[AcquisitionData::ACQUIS_NOTE.name]
  ...
  def acquis_provisos_input_locator
    text_area_locator([], AcquisitionData::ACQUIS_PROVISOS.name)
  end

  # Enters acquisition provisos
  # @param [Hash] data_set
  def enter_acquisition_provisos(data_set)
    acquis_provisos = data_set[AcquisitionData::ACQUIS_PROVISOS.name]
  ...
  def acquis_reason_input_locator
    text_area_locator([], AcquisitionData::ACQUIS_REASON.name)
  end

  # Enters an acquisition reason
  # @param [Hash] data_set
  def enter_acquisition_reason(data_set)
    acquis_reason = data_set[AcquisitionData::ACQUIS_REASON.name]
  ...
  def acquis_source_input_locator(index)
    input_locator([fieldset(AcquisitionData::ACQUIS_SOURCES.name, index)])
  end
  ...
  def select_acquisition_sources(data_set)
    ...
    acquis_sources && acquis_sources.each do |source|
      ...
      wait_for_element_and_click add_button_locator unless index.zero?
      enter_auto_complete(acquis_source_input_locator(index), source_options_locator, source[AcquisitionData::ACQUIS_SOURCE.name], 'Local Persons')
    end
  end

  # CREDIT LINE

  def credit_line_input_locator
    input_locator([], AcquisitionData::CREDIT_LINE.name)
  end
  ...
  def funding_source_input_locator(index)
    input_locator([fieldset(AcquisitionData::ACQUIS_FUNDING_LIST.name, index)], AcquisitionData::ACQUIS_FUNDING_SOURCE.name)
  end

  # Selects a set of funding sources
  # @param [Hash] data_set
  def select_funding_sources(data_set)
    fundings = data_set[AcquisitionData::ACQUIS_FUNDING_LIST.name]
    fundings && fundings.each do |funding|
      ...
      wait_for_element_and_click add_button_locator unless index.zero?
      enter_auto_complete(funding_source_input_locator(index), funding_source_options_locator, funding[AcquisitionData::ACQUIS_FUNDING_SOURCE.name], 'Local Persons')
    end
  end

  # FIELD COLLECTION EVENT NAME

  def event_names_input_locator(index)
    input_locator([fieldset(AcquisitionData::FIELD_COLLECT_EVENT_NAMES.name, index)])
  end
  ...
  def enter_obj_collection_info(data_set)
    ...
    field_collect_event_names && field_collect_event_names.each do |name|
      ...
      add_button_locator = add_button_locator([fieldset(AcquisitionData::FIELD_COLLECT_EVENT_NAMES.name)])
      wait_for_element_and_click add_button_locator unless index.zero?
      wait_for_element_and_type(event_names_input_locator(index), name[AcquisitionData::FIELD_COLLECT_EVENT_NAME.name])
    end
  end
