...
class PAHMASearchPage < SearchPage
  ...
  def enter_object_id_search_data(data_set)
    ...
    if object_nums
      ...
      if object_nums.instance_of? Array
        object_nums.each do |num|
          ...
          logger.debug "Entering #{} at index #{index}"
          wait_for_element_and_click object_num_add_btn unless index.zero?
          attempt_action(search_input_errors, num) { wait_for_element_and_type(object_num_input(index), num[PAHMAObjectData::OBJECT_NUM.name]) }
        end
      else
    ...
    components = data_set[PAHMAObjectData::IS_COMPONENT_GRP.name]
    components && components.each do |comp|
    ...
    obj_types && obj_types.each do |type|
    ...
    series_grp && series_grp.each do |series|
